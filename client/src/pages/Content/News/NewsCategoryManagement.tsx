...
const NewsCategoryManagement: React.FC = () => {
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from('news_categories').insert({ name: newCategoryName.trim(), is_active: true });
  ...
  const toggleActive = async (id: number, currentStatus: boolean) => {
    const { error } = await supabase.from('news_categories').update({ is_active: !currentStatus }).eq('id', id);
