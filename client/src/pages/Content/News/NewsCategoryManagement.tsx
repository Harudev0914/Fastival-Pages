...
const NewsCategoryManagement: React.FC = () => {
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from('news_categories').insert({ name: newCategoryName.trim() });
  ...
  const updateCategory = async (id: number) => {
    ...
    const { error } = await supabase.from('news_categories').update({ name: editingName.trim() }).eq('id', id);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '800px' }}>
      <div className="card" style={{ padding: '24px', backgroundColor: 'white', border: '1px solid rgb(226, 232, 240)', borderRadius: '16px', boxShadow: 'rgba(0, 0, 0, 0.02) 0px 4px 12px' }}>
        <h2 style={{ fontSize: '1.1rem', marginBottom: '16px', color: '#1e293b' }}>새 카테고리 추가</h2>
      ...
      <div className="card" style={{ padding: '0', overflow: 'hidden', border: '1px solid rgb(226, 232, 240)', borderRadius: '16px', backgroundColor: 'white' }}>
        {loading ? (
            ...
            <div style={{ width: '100%' }}>
                {categories.map((c) => (
                <div key={c.id} style={{ borderBottom: '1px solid #f1f5f9', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', '&:hover': { backgroundColor: '#f8fafc' } }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {editingId === c.id ? (
                            <button onClick={() => updateCategory(c.id)} style={{ padding: '8px 16px', backgroundColor: '#008b8b', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <Save size={16} /> 저장
                            </button>
                        ) : (
                            <>
                                <button onClick={() => { setEditingId(c.id); setEditingName(c.name); }} style={{ padding: '8px', cursor: 'pointer', border: 'none', background: '#f1f5f9', borderRadius: '6px', color: '#475569' }}><Edit2 size={16} /></button>
                                <button onClick={() => deleteCategory(c.id)} style={{ padding: '8px', cursor: 'pointer', border: 'none', background: '#fee2e2', borderRadius: '6px', color: '#dc2626' }}><Trash2 size={16} /></button>
                            </>
                        )}
                    </div>
                </div>
