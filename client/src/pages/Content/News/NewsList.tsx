...
const NewsList: React.FC = () => {
  ...
  const toggleActive = async (id: number, currentStatus: boolean) => {
    const { error } = await supabase.from('news').update({ is_active: !currentStatus }).eq('id', id);
    if (error) {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
                {loading ? (
                    ...
                    <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>등록된 뉴스가 없습니다.</td></tr>
                ) : (
                    news.map((item, index) => (
                    <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '16px 24px', textAlign: 'center' }}>{news.length - index}</td>
                        ...
                        <td style={{ padding: '16px 24px', textAlign: 'center' }}>
                            <button onClick={() => navigate(`/admin/dashboard/news/detail/${item.id}`)} style={{ padding: '6px', marginRight: '8px', cursor: 'pointer', border: 'none', background: 'none' }}><Edit2 size={16} /></button>
                            <button onClick={() => setModalConfig({isOpen: true, title: '삭제 확인', message: '정말 삭제하시겠습니까?', type: 'confirm', onConfirm: () => deleteNews(item.id)})} style={{ padding: '6px', color: '#dc2626', border: 'none', background: 'none' }}><Trash2 size={16} /></button>
                        </td>
                    </tr>
                    ))
                )}
