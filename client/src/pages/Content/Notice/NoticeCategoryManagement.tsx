...
const NoticeCategoryManagement: React.FC = () => {
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from('notice_categories').insert({ name: newCategoryName });
  ...
  const updateCategory = async (id: number) => {
    const { error } = await supabase.from('notice_categories').update({ name: editingName }).eq('id', id);
