...
const NoticeCategoryManagement: React.FC = () => {
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from('notice_categories').insert({ name: newCategoryName.trim(), is_active: true });
    if (error) console.error('Error adding notice category:', error);
    else {
      setNewCategoryName('');
      fetchCategories();
    }
  ...
  const updateCategory = async (id: number) => {
    ...
    const { error } = await supabase.from('notice_categories').update({ name: editingName.trim() }).eq('id', id);
    if (error) console.error('Error updating notice category:', error);
    else {
      setEditingId(null);
      fetchCategories();
    }
  ...
  const toggleActive = async (id: number, currentStatus: boolean) => {
    const { error } = await supabase.from('notice_categories').update({ is_active: !currentStatus }).eq('id', id);
  ...
  const deleteCategory = async (id: number) => {
    ...
    if (error) console.error('Error deleting notice category:', error);
    else fetchCategories();
  };
...
export default NoticeCategoryManagement;
