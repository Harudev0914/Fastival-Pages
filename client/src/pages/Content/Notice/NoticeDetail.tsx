...
const NoticeDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: catData } = await supabase.from('notice_categories').select('id, name');
  ...
  const saveNotice = async () => {
    ...
    if (id === 'new') {
        ...
        const { error: updateError } = await supabase.from('notices').update(payload).eq('id', id);
        error = updateError;
    }
    ...
    if (error) {
        ...
        setTimeout(() => navigate('/admin/dashboard/notices'), 1000);
    }
    setLoading(false);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="card" style={{ padding: '24px', backgroundColor: 'white', border: '1px solid rgb(226, 232, 240)', borderRadius: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            <button onClick={() => navigate('/admin/dashboard/notices')} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><ArrowLeft size={18}/> 목록으로</button>
            <button onClick={saveNotice} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', backgroundColor: '#008b8b', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>
