...
const CategoryManagement: React.FC = () => {
  ...
  const addCategory = async () => {
    ...
    const { error } = await supabase.from('rental_categories').insert({ name: newCategoryName.trim(), is_active: true });
  ...
  const updateCategory = async (id: number) => {
    ...
    const { error } = await supabase.from('rental_categories').update({ name: editingName.trim() }).eq('id', id);
  ...
  const toggleActive = async (id: number, currentStatus: boolean) => {
    const { error } = await supabase.from('rental_categories').update({ is_active: !currentStatus }).eq('id', id);
  ...
  return (
    <div style={{ padding: '0 20px', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div className="card" style={{ padding: '24px', backgroundColor: 'white', border: '1px solid rgb(226, 232, 240)', borderRadius: '16px', boxShadow: 'rgba(0, 0, 0, 0.02) 0px 4px 12px' }}>
        <h2 style={{ fontSize: '1.1rem', marginBottom: '16px', color: '#1e293b' }}>렌탈 카테고리 등록</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
                {loading ? (
                    ...
                    <tr><td colSpan={4} style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>등록된 카테고리가 없습니다.</td></tr>
                ) : (
