...
const RentalProductListPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const [{ data: p }, { data: b }, { data: c }] = [await productApi.list(), await brandApi.listActive(), await rentalCategoryApi.list()];
  ...
  const view = useMemo(() => {
    ...
    if (sort === 'price_low') v = [...v].sort((a, b) => Number(a.daily_price) - Number(b.daily_price));
    ...
    return v;
  }, [products, brandFilter, catFilter, sort]);
